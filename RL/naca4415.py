...
# NACA 4415 airfoil coordinates (x, y) from airfoiltools.com
...
# Parameters
chord_length = 1.0  # Chord length (adjust as needed)
span = 0.2  # Wing span (adjust as needed)

# Scale airfoil by chord length
airfoil_coords_scaled = airfoil_coords * chord_length

# Create 3D vertices by extruding the 2D profile
n_points = len(airfoil_coords_scaled)
vertices = []

# Front face (z=0)
for point in airfoil_coords_scaled:
...
# Front endcap (triangulate)
for i in range(1, n_points - 1):
    faces.append([0, i, i + 1])

# Back endcap (triangulate)
for i in range(1, n_points - 1):
...
# Save to STL file
airfoil_mesh.save('naca4415.stl')
print(f"NACA 4415 airfoil STL generated successfully!")
print(f"Chord length: {chord_length}, Span: {span}")
